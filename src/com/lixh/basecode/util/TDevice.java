...
public class TDevice {
	...
	public static int getDefaultLoadFactor() {
		if (_loadFactor == null) {
			Integer integer = Integer
					.valueOf(0xf & BaseApplication.getInstance().getResources()
	...
	public static boolean hasInternet() {
		boolean flag;
		if (((ConnectivityManager) BaseApplication.getInstance()
	...
	public static boolean isPackageExist(String pckName) {
		try {
			PackageInfo pckInfo = BaseApplication.getInstance()
	...
	public static void gotoMarket(Context context, String pck) {
		if (!isHaveMarket(context)) {
			Alert.displayToastForLengthShort(context, "你手机中没有安装应用市场！");
			return;
		}
		Intent intent = new Intent();
		intent.setAction(Intent.ACTION_VIEW);
	...
	public static void copyTextToBoard(String string) {
		if (TextUtils.isEmpty(string))
			return;
		ClipboardManager clip = (ClipboardManager) BaseApplication
				.getInstance().getSystemService(Context.CLIPBOARD_SERVICE);
		clip.setText(string);
		Alert.displayToastForLengthShort(BaseApplication.getInstance()
